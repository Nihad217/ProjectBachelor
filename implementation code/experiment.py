...
class ExperimentApp:
    ...
    def calibrate_glasses(self):
        ...
        frameAndGaze=self.device.recieve_matched_scene_video_frame_and_gaze(timeout_seconds=1/15)

        if frameAndGaze is None:
            print("keine gaze-Daten empfangen! ")
            self.marker_window.hideMarkers()
        return
    ...
    def setup_slide1(self):
        ...
        try:
            image = Image.open("pictures/waldo.jpg").resize((350, 350))
        ...
        pause_label = Label(self.slidePause, text="Pause. Drücke weiter, um fortzufahren",
                            font=("Segoe UI", 18))
    ...
    def load_images(self):
        for i in range(2, 12):
            try:
                ...
                img = Image.open(image_path).resize((1000, 1000))
